...
def projected_record():
    ...
    for week in range(1, 15):
        req = requests.get(f'https://api.sleeper.app/projections/nfl/2022/{week}/?season_type=regular&position[]=DEF&position[]=QB&position[]=RB&position[]=TE&position[]=WR&position[]=K&order_by=pts_ppr')
        reqData = json.loads(req.text)
        for data in reqData:
            player_id = data["player_id"]
            for roster in roster_and_players.items():
                if player_id in roster[1]:
                    predicted_score = calculate_score(data["stats"])
                    sql_string = f'INSERT INTO cr_league_data (playerID, week, roster_id, predicted_score, pos) VALUES ' \
                                 f'(\'{player_id}\', \'{data["week"]}\', \'{roster[0]}\', \'{predicted_score}\',' \
                                 f' \'{data["player"]["position"]}\');'
                    DataBaseHandle.update_db(sql_string)

        calculate_best_roster_score(week)
...
def calculate_best_roster_score(week):
    # Calculate best roster score by week
    # Roster = 2 QBs, 2 RBs, 3 WRs, TE, FLEX (W/R/T), DEF, Kicker
    for i in range(1, 11):
        roster_data = DataBaseHandle.get_data_by_roster_id_and_week(i, week)
        # the_qbs = [player[3] for player in roster_data]
        the_qbs = sorted([float(player[3]) if player[4] == 'QB' else 0 for player in roster_data], reverse=True)
        qbs = the_qbs[0] + the_qbs[1]
        the_rbs = sorted([float(player[3]) if player[4] == 'RB' else 0 for player in roster_data], reverse=True)
        rbs = the_rbs[0] + the_rbs[1]
        the_wrs = sorted([float(player[3]) if player[4] == 'WR' else 0 for player in roster_data], reverse=True)
        wrs = the_wrs[0] + the_wrs[1] + the_wrs[2]
        the_tes = sorted([float(player[3]) if player[4] == 'TE' else 0 for player in roster_data], reverse=True)
        tes = the_tes[0]
        the_flexs = sorted([float(player[3]) if player[4] == 'RB' or player[4] == 'WR' or player[4] == 'TE' else 0 for player in roster_data], reverse=True)
        remove_list = [the_rbs[0], the_rbs[1], the_wrs[0], the_wrs[1], the_wrs[2]]
        for item in remove_list:
            the_flexs.remove(item)
        flex = the_flexs[0]
        the_ds = sorted([float(player[3]) if player[4] == 'DEF' else 0 for player in roster_data], reverse=True)
        d = the_ds[0]
        the_ks = sorted([float(player[3]) if player[4] == 'K' else 0 for player in roster_data], reverse=True)
        k = the_ks[0]
        roster_total = round(sum([qbs, rbs, wrs, tes, flex, d, k]), 2)
        roster_sql_string = f'INSERT INTO cr_league_data_roster (roster_id, week, predicted_score) VALUES (\'{i}\', \'{week}\', \'{roster_total}\');'
        DataBaseHandle.update_db(roster_sql_string)
